...
- Uses two on-screen photodiode trigger images for EEG timing:
  one trigger for left-attend trials and one trigger for right-attend trials
...
# Frequencies requested for the updated version
LEFT_FREQ = 9                # Hz
RIGHT_FREQ = 14              # Hz

FIXATION_DUR = 1.0           # seconds
CUE_DUR = 0.8                # seconds
STIM_DUR = 20.0              # seconds; updated from previous shorter version
ITI_DUR = 1.2                # seconds

N_BLOCKS = 4
TRIALS_PER_BLOCK = 10

# Keep this off for the current pure SSVEP/trigger setup.
...
# Stimulus layout
# The actual left/right positions are computed after window creation
# so the squares are placed as far apart as safely possible.
MAX_STIM_X_OFFSET = 650      # max distance from center in pixels
STIM_EDGE_MARGIN_X = 80      # safety margin from screen edge
STIM_SIZE = (220, 220)
TARGET_LINE_WIDTH = 4
CUE_LINE_WIDTH = 8
FIXATION_SIZE = 30
...
# These use units="height", exactly like the previous working experiment.
# You now have two trigger locations: one left, one right.
LEFT_PHOTODIODE_POS = (-0.7, -0.4)
RIGHT_PHOTODIODE_POS = (0.7, -0.4)
PHOTODIODE_SIZE = 0.05

# Trigger pulse duration in frames.
# At nominal 60 Hz, 2 frames is about 33.3 ms.
PHOTODIODE_ON_FRAMES = 2

# Trigger pulse repeat interval during stimulation.
# Pulse happens at t = 0, 1, 2, 3, ... seconds until the end of stimulation.
...
def square_wave_state(frameN, freq_hz, refresh_hz):
    ...
    It samples a 50% duty-cycle square wave once per display frame.
    """
...
def draw_photodiodes(active_side=None, trigger_state="off"):
    """Draw both photodiode trigger images.

    active_side:
        None    -> both triggers off
        'left'  -> left trigger can be on/off; right stays off
        'right' -> right trigger can be on/off; left stays off
    ...
    # Left trigger
    if active_side == 'left' and trigger_state == "on":
        left_trigger_on.draw()
    else:
        left_trigger_off.draw()

    # Right trigger
    if active_side == 'right' and trigger_state == "on":
        right_trigger_on.draw()
    else:
        right_trigger_off.draw()
...
def make_block_trials(block_num):
    # Balance attended side within each block
...
# The monitor may be measured as ~58.9–59.9 Hz, but the stimulus is designed for 60 Hz.
refresh_hz = 60
...
# This pushes the two flickering squares far apart while keeping them safely on screen.
...
# =========================
left_trigger_on = visual.ImageStim(
    win,
    image=trigger_on_path,
    pos=LEFT_PHOTODIODE_POS,
    size=PHOTODIODE_SIZE,
    units="height"
)

left_trigger_off = visual.ImageStim(
    win,
...
right_trigger_on = visual.ImageStim(
    win,
    image=trigger_on_path,
    pos=RIGHT_PHOTODIODE_POS,
    size=PHOTODIODE_SIZE,
    units="height"
)

right_trigger_off = visual.ImageStim(
    win,
    image=trigger_off_path,
    pos=RIGHT_PHOTODIODE_POS,
    size=PHOTODIODE_SIZE,
...
with open(meta_path, 'w', encoding='utf-8') as meta_file:
    ...
    meta_file.write(f"Left photodiode position: {LEFT_PHOTODIODE_POS}\n")
    meta_file.write(f"Right photodiode position: {RIGHT_PHOTODIODE_POS}\n")
...
for block_num in range(1, N_BLOCKS + 1):
    ...
    for trial in block_trials:
        ...
        for frameN in range(total_frames):
            ...
            # Photodiode pulse logic:
            # - Pulse at stimulation onset
            # - Then pulse once every PHOTODIODE_PULSE_INTERVAL_S seconds
            # - Only the trigger corresponding to the attended side pulses
            ...
            if USE_PHOTODIODE_PATCH and photodiode_pulse_on:
                draw_photodiodes(active_side=attend_side, trigger_state="on")
            else:
                draw_photodiodes(active_side=None, trigger_state="off")
        ...
        writer.writerow({
            ...
            'photodiode_trigger_side': attend_side,
